...
*    inticates how many timer that SIGNAL was received
* void set_signals()
*    whenever BREAK_SIGNAL or DUMP_SIGNAL is received, increase
...
{struct sigaction act;
    ...
    if(sigaction(BREAK_SIGNAL, &act, NULL)){
        report(R_fatal,"Cannot set signal " mkstringof(BREAK_SIGNAL) ", aborting\n");
        return 1;
    }
...
*     execute the outer approximation algorithm
*/
...
{int r;
    ...
    switch(outer()){ // execute the algorithm
