...
#define DEF_ShuffleMatrix	1	/* yes */
...
/* vertex pool */
#define DEF_FacetPoolSize	0	/* don't use facet pool */
...
#define DEF_VertexRecalcEps	1e-6
/* Reporting */
...
"#    of adjacent facets.\n"
"#\n"
CFG( RecalculateVertices, INTEGER)
"#    after that many iterations recalculate all vertex coordinates\n"
"#    from the set of adjacent facets. Should be zero (never), or at\n"
"#    least 5.\n"
"#\n"
...
"#    stop processing as if received a "  mkstringof(BREAK_SIGNAL) " signal. Zero\n"
...
"#    processing as if received a " mkstringof(BREAK_SIGNAL) " signal. Zero means unlimited,\n"
"#    otherwise should be at least 60.\n"
"#\n"
CFG( FacetPoolSize, INTEGER)
"#    size of the facet pool; add the facet to the approximation\n"
"#    which discards the largest number of existing vertices. Should\n"
"#    be zero (don't use it) or at least 5. Using facet pool makes\n"
"#    additional oracle calls, but can simplify the approximating\n"
"#    polytopes. See OracleCallLimit. Should be less than " mkstringof(MAX_FACET_POOL) ".\n"
"#\n"
...
"#    iteration when filling the facet pool. Zero means no limit;\n"
...
"#\n"
CFG( RoundFacets, BOOL)
"#    when the oracle reports a result facet, round its coordinates\n"
...
"#    print out resources used (number of iterations, edge tests, etc.)\n"
"#\n"
...
"#\n"
"#" CFG( LineqEps, REAL)
"#    when solving a system of linear equations for vertex coordinates,\n"
"#    a value smaller than this is considered to be zero.\n"
"#\n"
"#" CFG( RoundEps, REAL)
"#    if facets reported by the Oracle are rounded (RoundFacets=1), the\n"
"#    tolerance in the rounding algorithm.\n"
"#\n"
"#" CFG( VertexRecalcEps, REAL)
"#    when recalculating vertices, report numerical instability when\n"
"#    the new and old values differ at least that much.\n"
"#\n"
"# *** end of " PROGNAME ".cfg ***\n\n";
...
"  --boot=<vertex-file>\n"
"                   start the algorithm with these facets\n"
...
"  boot       specify a set of precomputed facets\n"
...
"Vertices of an intermediate approximation not known to be final ones\n"
"are printed in lines starting with lower case v.\n"
"\n"
...
"lines, and repeats the problem's dimensions as\n"
...
"  5   program execution aborted by a " mkstringof(BREAK_SIGNAL) " signal\n"
"\n"
);}
...
"******************************\n"
"\n"
"Only " mkstringof(BREAK_SIGNAL) " and " mkstringof(DUMP_SIGNAL) " signals are processed:\n"
"The " mkstringof(BREAK_SIGNAL) " signal stops the algorithm. If ExtractAfterBreak=1 (default),\n"
"  then continue extracting new vertices by asking the oracle about\n"
"  every vertex of the actual approximation, but don't process the newly\n"
"  obtained facets. This method might miss several extremal solutions.\n"
"  A second " mkstringof(BREAK_SIGNAL) " signal aborts this post-processing. The program's exit\n"
...
"The frequency is determined by the value of the keyword \"--CheckPoint\" which\n"
"defaults to 10000 seconds. The checkpoint file name is created from the\n"
"filestub by appending `NNN.chk', where NNN starts at 000 and increases by\n"
"one for each iteration. Computation can be resumed by calling the program\n"
"with the `--resume=<checkpoint-file>' option. See also the description of\n"
"the 'CheckPoint' keyword in the config file.\n"
);}
...
"******************************\n"
"\n"
"Start the algorithm with a given set of facets. When a partial list of\n"
"the facets is known, the program can be instructed to add them first\n"
"before calling the oracle for other facets. The file containing these\n"
"facet equations is specified as `--boot=<file>'. Facets are specified one\n"
"per line; the format is the same as in the output. The first character\n"
"in the line is an upper case F followed by d+1 (number of objectives)\n"
"fractions or floating point numerals such as\n"
"    F 0 5/2 3/4 7.123456789 -1/2\n"
"Lines not starting with upper case F are ignored. All facets are\n"
"processed without checking their extremality. In higher dimensions\n"
"the running time is very sensitive to the order facets are prsented\n"
"- even a tenfold speed-up or slow-down is possible.\n"
);}
...
"using glpk " mkstringof( GLP_MAJOR_VERSION.GLP_MINOR_VERSION)
" (GNU Linear Programming Kit).\n"
"It is an implementation of Benson's outer approximation algorithm.\n"
COPYRIGHT "\n"
);}
...
static struct int_params {
  ...
  CFG(RecalculateVertices,5,1000000),
  CFG(CheckConsistency,5,1000000),
  CFG(FacetPoolSize,5,MAX_FACET_POOL),
...
//    CFG(TimeLimit);		/* time limit in seconds */
    CFG(FacetPoolSize);		/* use facet pool */
    CFG(OracleCallLimit);	/* oracle call limit per iteration */
    CFG(RecalculateVertices);	/* how ofter recalculate vertices */
#undef CFG
    /* double parameters */
#define CFG(x)	\
    ...
    CFG(RoundEps);		/* tolerance for RoundFacets */
#undef CFG
}
